...
function alertContents() {
    if (http_request.readyState == 4) {
        if (http_request.status == 200) {
            ...
            let xml = http_request.responseXML;
            let canciones = xml.getElementsByTagName("cancion")
            for(var indice=0; indice < canciones.length; indice++){
                var li = document.createElement("li");
                var texto = document.createTextNode(canciones[indice].getAttribute("titulo"))
